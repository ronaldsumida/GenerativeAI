...
# Helper function for running a streaming chat with potential tool calls
def run_streaming_chat(client, model, messages, tools=None, tools_modules=None, max_iterations=10):
    for _ in range(max_iterations):
                ...
                if delta and delta.tool_calls:
                    for tc_delta in delta.tool_calls:
                        if len(tool_calls) <= tc_delta.index:
                            tool_calls.append({'type': 'function', 'function': {'arguments': '', 'name': ''}})
        ...
        # Call the tools that the model requested and append
        # each result to the chat
        if tool_calls:
            ...
            # Loop back to send the results of the tool call(s)
            # to the LLM and check for more tool calls
            continue
...
def exec_tool_call(function_name, function_args, tools_modules):
    try:
